...
### PRDT replicated domain object

PRDT 流の *replicated domain object*（純粋なドメイン状態機械 + 複製 finalization protocol、証明付き
compaction、quorum closure、Why3/Z3 契約）はこのリポジトリで開発した後、独立 module
[`mizchi/prdt`](https://github.com/mizchi/prdt) に切り出した。`Hasher` / `Signer` / `Verifier`
trait は同じ形なので、一行の adapter で組み合わせられる。
