...
import {
  ...
  verifyEvidenceCaseDismissalCertificateAsync,
} from "./evidence-case-dismissal-certificate";
import {
  decodeLineageDecisionCertificate,
  verifyLineageDecisionCertificate,
  verifyLineageDecisionCertificateAsync,
  type AsyncLineageDecisionDigestAdapter,
  type AsyncLineageDecisionVerifierRegistry,
...
export interface AsyncReferenceGameEvidenceResolutionAuthorizationOptions {
  roster: LineageDecisionArbiterRoster;
...
export function createAsyncReferenceGameEvidenceResolutionAuthorizationVerifier(
  options: AsyncReferenceGameEvidenceResolutionAuthorizationOptions,
): EvidenceCaseResolutionAuthorizationVerifier {
  return Object.freeze({
    async verify(notice: EvidenceCaseResolutionNotice): Promise<boolean> {
      const authorization = recordValue(notice.authorization);
      if (!authorization || !("certificate" in authorization)) return false;
      if (authorization.kind === "dismissal") {
        if (notice.resolution.decision !== "dismissed") return false;
        ...
        if (!certificate) return false;
        const verified = await verifyEvidenceCaseDismissalCertificateAsync(
          certificate,
          {
            expectedScope: notice.scope,
            expectedUnit: notice.unit,
      ...
      if (authorization.kind === "lineage_decision") {
        ...
        if (!certificate || certificate.statement.version !== 2) return false;
        const verified = await verifyLineageDecisionCertificateAsync(
          certificate,
          {
            expectedScope: notice.scope,
            expectedUnit: notice.unit,
