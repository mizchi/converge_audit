...
検証済みanchorからowner/version/event境界を追跡し、標準SHA-256で並列再計算する。origin receipt/rootと
authenticated-map membership/public state root再計算は未監査backendのままである。
...
| lineage derived identity and adjudication | open-world lineage proofの保存ID、authority checkpoint + replay-witness certificate、4件×transferの内部認証、anchor-bound transition root、reference/open-world decision、reference dismissal、evidence-source proposal/resolutionは同期MoonBit backendと非同期WebCrypto backendが一致した場合だけ永続化する | MoonBit canonical authentication transcript + cached async WebCrypto verifier + Unicode framing/broken-backend negative control + workerd integration | Tested locally。lineage状態遷移はQuintでmodel checked、origin root・authenticated-map/public state rootと暗号強度は未解決 | `just formal-check` + `pnpm --dir examples/cf-game-audit test` |
