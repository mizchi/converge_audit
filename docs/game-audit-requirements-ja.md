...
| inventory lineage identity and adjudication | open-world lineage proofの保存ID、authority checkpoint + replay-witness certificate/session manifest、origin receipt/initial root、authenticated-map/public state root、4件×transferの内部認証、anchor-bound transition root、reference/open-world decision、reference dismissal、evidence-source proposal/resolutionは同期MoonBit backendと非同期WebCrypto backendが一致した場合だけ永続化する | MoonBit canonical authentication/manifest transcript + cached async WebCrypto verifier + Unicode framing/broken-backend negative control + workerd integration | Tested locally。lineage状態遷移はQuintでmodel checked。event/asset-delta意味論は`n-f` witnessへ委譲し、中央再計算は非要件。暗号強度は未解決 | `just formal-check` + `pnpm --dir examples/cf-game-audit test` |
